...
export const ReservationForm: React.FC<ReservationFormProps> = ({
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6 max-w-4xl mx-auto min-h-screen sm:min-h-0">
      ...
      <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
          <div>
            <label className="flex items-center gap-2 text-sm font-medium text-gray-700 mb-1 sm:mb-2">
              <User size={16} />
          ...
          <div>
            ...
            <input
              ...
              } py-2.5 sm:py-2 text-base sm:text-sm`}
              required
            />
            {dateError && (
          ...
          <div className="sm:col-span-2">
            ...
            <textarea
              ...
              className="w-full px-3 py-2.5 sm:py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 resize-none text-base sm:text-sm"
              rows={3}
              dir={isRTL ? 'rtl' : 'ltr'}
            />
          </div>
        ...
        {!dateError && timeSlots.length > 0 && (
          <>
            <div className="space-y-3 sm:space-y-4">
              <h3 className="text-base sm:text-lg font-semibold text-gray-800">Select Seating Area</h3>
              <div className="grid grid-cols-3 gap-2 sm:flex sm:gap-2">
                {(['bar', 'inside', 'outside'] as const).map(area => (
                  <button
                    ...
                    className={`px-3 py-2.5 sm:px-4 sm:py-2 rounded-lg font-medium transition-colors text-sm sm:text-base ${
                      selectedArea === area
            ...
            {selectedTables.length > 0 && (
              <div className="bg-gray-50 rounded-lg p-3 sm:p-4">
                <h4 className="font-semibold text-gray-800 mb-2 text-sm sm:text-base">Reservation Summary</h4>
                <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 sm:gap-4 text-xs sm:text-sm">
                  <div>
                    <span className="text-gray-600">Party Size:</span>
                    <span className="font-medium ml-1 sm:ml-2">{formData.partySize}</span>
                  </div>
                  <div>
                    <span className="text-gray-600">Total Capacity:</span>
                    <span className="font-medium ml-1 sm:ml-2">{getTotalCapacity()}</span>
                  </div>
                  <div>
                    <span className="text-gray-600">Duration:</span>
                    <span className="font-medium ml-1 sm:ml-2 block sm:inline">{formData.startTime} - {endTime}</span>
                  </div>
                  <div>
                    <span className="text-gray-600">Tables:</span>
                    <span className="font-medium ml-1 sm:ml-2">{selectedTables.length}</span>
                  </div>
                </div>
              </div>
            )}
        ...
        {/* Actions */}
        <div className="flex flex-col sm:flex-row justify-end gap-3 pt-4 border-t border-gray-200 pb-safe">
          <button
            type="button"
            onClick={onCancel}
            className="w-full sm:w-auto px-6 py-3 sm:py-2 text-gray-700 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors text-base sm:text-sm"
          >
            {t('actions.cancel')}
          </button>
          <button
            ...
            className="w-full sm:w-auto flex items-center justify-center gap-2 px-6 py-3 sm:py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:bg-gray-400 disabled:cursor-not-allowed transition-colors text-base sm:text-sm"
          >
            <Save size={16} />
            {t('actions.save')}
          </button>
